 ...
 */
public class AusgabenKatalogController implements Initializable, CloseRequestable {
    private final static Logger LOGGER = Logger.getLogger(AusgabenKatalogController.class.getName());
    ...
    private Comparator<AusgabePosition> ausgabenComparator = (ausgabePos1, ausgabePos2) ->
            ausgabePos1.getAusgabe().getDatum().compareTo(ausgabePos2.getAusgabe().getDatum());
    ...
    public void handleSuchenAction(ActionEvent event) {
        ...
        Collection<AusgabePosition> ausgabPositionColl = HausJpaPersistence.getInstance().selectAusgabePositionen(bedingungen);
        ArrayList<AusgabePosition> positionenList = new ArrayList<>(ausgabPositionColl);
        Collections.sort(positionenList, ausgabenComparator);
        ausgabePositionen.clear();
        ausgabePositionen.addAll(positionenList);
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        zeigeAusgabenLaufendesJahr();

        pruefeButtons();

    }

    private void zeigeAusgabenLaufendesJahr() {
    ...
    private void prepareKategorieBoxes(ChoiceBox<Operator> operatorChoiceBox, ComboBox<String> argumentComboBox) {
        ...
        for (AusgabeArt ausgabeArt : AusgabeArt.values()) {
            argumentComboBox.getItems().add(ausgabeArt.getKurzBeschreibung());
        }
    ...
    private void initializeTableViewColumns() {
        datumCol.setCellValueFactory(
                cellData -> {
                    ...
                    property.setValue(ausgabePos.getAusgabe().getDatumInternational());
                    return property;
                });
        ...
        hausCol.setCellValueFactory(
                cellData -> {
                    SimpleStringProperty property = new SimpleStringProperty();
                    AusgabePosition ausgabePos = (AusgabePosition) ((TableColumn.CellDataFeatures) cellData).getValue();
                    property.setValue(ausgabePos.getAusgabe().getHaus().getName());
                    return property;
                });
